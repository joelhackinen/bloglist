...
import { removeUserBlog, appendUserBlog } from './usersReducer'
...
export const createBlog = (blogObject) => {
  return async dispatch => {
    try {
      ...
      dispatch(appendUserBlog({ blog: created, userId: blogObject.user.id }))
...
export const deleteBlog = (id, userId) => {
  return async dispatch => {
    try {
      ...
      dispatch(removeUserBlog({ userId, blogId: id }))
